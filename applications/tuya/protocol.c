...
//    #error "请自行完成RTC时钟写入代码,并删除该行"
   ...
   */
    ...
    rt_err_t ret = RT_EOK;
    /* 设置日期 */
    LOG_D("Set RTC date: %s %s %s",time[1],time[2],time[3]);
    ret = set_date(2000+atoi(time[1]), atoi(time[2]), atoi(time[3]));
    if (ret != RT_EOK)
    {
    ...
    /* 设置时间 */
    LOG_D("Set RTC time: %s %s %s",time[4],time[5],time[6]);
    ret = set_time(atoi(time[4]), atoi(time[5]), atoi(time[6]));
    if (ret != RT_EOK)
    {
