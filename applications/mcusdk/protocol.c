...
#define LOG_TAG     "TUYA_protocol.c"
...
//    #error "请在此处理可下发可上报数据及只上报数据示例,处理完成后删除该行"
    ...
    mcu_dp_enum_update(DPID_STATUS,当前开合状态); //枚举型数据上报;枚举值opened, closed
    mcu_dp_enum_update(DPID_CONTROL,当前设备控制); //枚举型数据上报; 枚举值open, stop, close, continue
    mcu_dp_bool_update(DPID_CLOSE_REMINDER,当前忘关提醒); //BOOL型数据上报;
    mcu_dp_value_update(DPID_PERCENT_CONTROL,当前开启百分比控制); //VALUE型数据上报;
    mcu_dp_value_update(DPID_PERCENT_STATE,当前开启百分比状态); //VALUE型数据上报;
    mcu_dp_value_update(DPID_TIME_TOTAL,当前全程运行时间); //VALUE型数据上报;
    mcu_dp_enum_update(DPID_WORK_MODE,当前工作模式); //枚举型数据上报;
    mcu_dp_bool_update(DPID_SWITCH_DOOR_WIN,当前门窗开关); //BOOL型数据上报;
    mcu_dp_enum_update(DPID_SMART_WEATHER,当前天气); //枚举型数据上报;枚举值cloudy, rainy, sunny
    mcu_dp_enum_update(DPID_WIND_SPEED_STATE,当前风速状态); //枚举型数据上报;枚举值no_wind, breezy, gale
    mcu_dp_value_update(DPID_TEMP_CURRENT,当前当前温度); //VALUE型数据上报;
    mcu_dp_bool_update(DPID_SWITCH_SENSOR,当前风光雨传感器开关); //BOOL型数据上报;
    mcu_dp_value_update(DPID_HUMUDITY,当前相对湿度); //VALUE型数据上报;
    */
    int status;
    ...
    mcu_dp_enum_update(DPID_STATUS,status);
    mcu_dp_value_update(DPID_TEMP_CURRENT,curr_property.IndoorTemperature); //VALUE型数据上报;
    mcu_dp_value_update(DPID_HUMUDITY,curr_property.IndoorHumidity); //VALUE型数据上报;
}
...
{
    //示例:当前DP类型为ENUM, 枚举值open, stop, close, continue
    ...
    switch(control) {
        ...
        case 3:
            re = rt_event_send(screen_event, RESUME_EVT_FLG);
          LOG_D("Send RESUME EVENT, result : %d", re );
          re = rt_event_send(ctrl_event, RESUME_EVT_FLG);
          LOG_D("Send RESUME EVENT, result : %d", re );
        break;
...
/*****************************************************************************
函数名称 : dp_download_close_reminder_handle
功能描述 : 针对DPID_CLOSE_REMINDER的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_close_reminder_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    unsigned char close_reminder;
    
    close_reminder = mcu_get_dp_download_bool(value,length);
    if(close_reminder == 0) {
        //开关关
    }else {
        //开关开
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_CLOSE_REMINDER,close_reminder);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_switch_door_win_handle
功能描述 : 针对DPID_SWITCH_DOOR_WIN的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_switch_door_win_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    unsigned char switch_door_win;
    
    switch_door_win = mcu_get_dp_download_bool(value,length);
    if(switch_door_win == 0) {
        //开关关
    }else {
        //开关开
    }
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_SWITCH_DOOR_WIN,switch_door_win);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_switch_sensor_handle
功能描述 : 针对DPID_SWITCH_SENSOR的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_switch_sensor_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    unsigned char switch_sensor;
    
    switch_sensor = mcu_get_dp_download_bool(value,length);
    if(switch_sensor == 0) {
        //开关关
    }else {
        //开关开
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_SWITCH_SENSOR,switch_sensor);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    switch(dpid) {
        ...
        break;
        case DPID_CLOSE_REMINDER:
            //忘关提醒处理函数
            ret = dp_download_close_reminder_handle(value,length);
        break;
        ...
        break;
        case DPID_SWITCH_DOOR_WIN:
            //门窗开关处理函数
            ret = dp_download_switch_door_win_handle(value,length);
        break;
        case DPID_SWITCH_SENSOR:
            //风光雨传感器开关处理函数
            ret = dp_download_switch_sensor_handle(value,length);
        break;
...
//    #error "请自行完成相关代码,并删除该行"
    ...
    */
    if(time[0] == 1) {
        //正确接收到wifi模块返回的格林数据
        LOG_D("Get green date: %s %s %s",time[1],time[2],time[3]);
        LOG_D("Get green time: %s %s %s",time[4],time[5],time[6]);
...
//    #error "请自行完成RTC时钟写入代码,并删除该行"
   ...
   */
    ...
    rt_err_t ret = RT_EOK;
    /* 设置日期 */
    LOG_D("Set RTC date: %d %d %d",2000+time[1],time[2],time[3]);
    ret = set_date(2000+time[1], time[2], time[3]);
    if (ret != RT_EOK)
    {
    ...
    /* 设置时间 */
    LOG_D("Set RTC time: %d %d %d",time[4],time[5],time[6]);
    ret = set_time(time[4], time[5], time[6]);
    if (ret != RT_EOK)
    {
        ...
        LOG_E("Failed to get date time, probably network is not ready. ");
    }
}
#endif
...
//    #error "请自行实现wifi功能测试成功/失败代码,完成后请删除该行"
    if(result == 0) {
        ...
        if(rssi == 0x00) {
            ...
            LOG_E("Failed to find router name:tuya_mdev_test, please check again.");
        }else if(rssi == 0x01) {
            //模块未授权
            LOG_E("Module is not authorized.");
        }
    }else {
        //测试成功
        //rssi为信号强度(0-100, 0信号最差，100信号最强)
        LOG_D("Wifi test success. rssi:%d (100 is the best)", rssi);
    }
}
#endif
...
//    #error "请自行完成打开天气功能返回数据处理代码,完成后请删除该行"
    ...
    if(res == 1) {
        //打开天气返回成功
        LOG_D("weather open success.");
//        for (int i = 0; i < strlen(res); i++)
//        {
//            rt_kprintf("%02x",&res+i);
//        }
//        rt_kprintf("\r\n");
    }else if(res == 0) {
        //打开天气返回失败
        //获取错误码
        err_num = err; 
        LOG_E("weather open failed with error code:%d", err_num);
    }
}
...
//    #error "这里仅给出示例，请自行完善天气数据处理代码,完成后请删除该行"
    ...
    if(my_strcmp(name, "temp") == 0) {
        LOG_D("day:%d temp value is:%d", day, value_int);          //int 型
        rt_enter_critical();
        curr_property.OutdoorTemperature = value_int;
        rt_exit_critical();
    }else if(my_strcmp(name, "humidity") == 0) {
        LOG_D("day:%d humidity value is:%d", day, value_int);      //int 型
        rt_enter_critical();
        curr_property.OutdoorHumidity = value_int;
        rt_exit_critical();
    }else if(my_strcmp(name, "pm25") == 0) {
        LOG_D("day:%d pm25 value is:%d", day, value_int);          //int 型
        rt_enter_critical();
        curr_property.Outdoor_pm25 = value_int;
        rt_exit_critical();
    }else if(my_strcmp(name, "condition") == 0) {
        LOG_D("day:%d condition value is:%s", day, value_string);  //string 型
    }
}
#endif
...
//    #error "请自行实现模块时间服务通知结果代码,完成后请删除该行"
    ...
    switch(sub_cmd) {
        case 0x01: { //子命令  打开模块时间服务通知
            ...
            if(value[1] == 0) {
                //服务开启成功
                LOG_D("Time Server Enabled Success.");
            }else {
                //服务开启失败
                LOG_E("Time Server Enabled Failed.");
            }
        }
        break;
